...
pub(super) fn match_num_channels(samples: &Vec<u8>, num_channels: usize, export_settings: WavSettings) -> Vec<u8> {
    ...
    for k in (0..samples.len()).step_by(export_settings.bytes_per_sample) {
        let channel_idx = k / export_settings.bytes_per_sample % num_channels;
        ...
        if channel_idx + 1 == num_channels && num_channels < export_settings.num_channels {
            for _ in 0..((export_settings.num_channels - num_channels) * export_settings.bytes_per_sample) {
                output.push(0);
            }
        }
    }
