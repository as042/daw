pub fn sample_to_f64(sample: [u8; 8], bytes_per_sample: i32) -> f64 {
    ...
    let final_value = unsigned * 2.0 - 1.0;
...
pub fn f64_to_sample(value: f64, bytes_per_sample: i32) -> [u8; 8] {
    ...
    let int = (value + 1.0) * 0.5 * sample_max;
    let final_value = int.clamp(sample_max * -1.0, sample_max - 1.0) as i64;

    final_value.to_le_bytes()
}

#[cfg(test)]
#[test]
fn test_sample_to_f64() {
    let sample = [0, 128, 0, 0, 0, 0, 0, 0];
